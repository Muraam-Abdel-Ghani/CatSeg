""" PyConv networks for image recognition as presented in our paper:
    ...
    Code From: https://github.com/iduta/pyconv/blob/master/models/pyconvresnet.py
"""
import torch
...
def pyconvresnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load("/notebooks/TinyToolSegNet/model/pretrained/pyconvresnet50.pth"))
    return model
...
def pyconvresnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load("/notebooks/TinyToolSegNet/model/pretrained/pyconvresnet101.pth"))
    return model
...
def pyconvresnet152(pretrained=False, **kwargs):
    ...
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        # model.load_state_dict(torch.load("/notebooks/TinyToolSegNet/model/pretrained/pyconvresnet152.pth"))
        model.load_state_dict(torch.load("C:\\Users\\muraa\\Documents\\University\\Masters\\Independent Study\\Cataract-1K Semantic Surgical Scene Segmentation\\Models\\CatSeg - PFDNet - MSIFA_DW\\pyconvresnet pretrained weights\\pyconvresnet152.pth")) 
    return model